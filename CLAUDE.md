...
Color Challenge is a Minecraft mod where players compete to earn 10,000 gold by mining and selling ores to an NPC staff. Built with **Architectury** for cross-platform support (Fabric + NeoForge/Forge), targeting Minecraft 1.21.11, 1.21.10, 1.21.9, 1.21.8, 1.21.7, 1.21.6, 1.21.5, 1.21.4, 1.21.3, 1.21.1 (Java 21) and 1.20.1 (Java 17).
...
- **`common/shared/`** — Platform-independent shared code (no build.gradle, included as srcDir by common module). Base package: `com.colorchallenge`
- **`common/1.21.1/`** — Version-specific common module for MC 1.21.1 (Architectury common). Includes `common/shared` sources via `srcDir`.
- **`common/1.21.11/`** — Version-specific common module for MC 1.21.11. Contains all sources (does NOT include `common/shared` via srcDir due to 1.21.2+ breaking API changes).
...
- **`fabric/<version>/`** — Fabric platform build module for each MC version. Maps to Gradle project `:fabric`.
- **`neoforge/base/`** — NeoForge platform base code (no build.gradle, included as srcDir by neoforge module)
- **`neoforge/<version>/`** — NeoForge platform build module for each MC version (1.21.1+). Maps to Gradle project `:neoforge`.
...
`settings.gradle` dynamically resolves modules based on `target_mc_version`. Gradle project names are unified (`:common`, `:fabric`, `:neoforge`, `:forge`) regardless of version:
- `:common` → `common/<version>/`
- `:fabric` → `fabric/<version>/`
- `:neoforge` → `neoforge/<version>/` (1.21.1+)
- `:forge` → `forge/<version>/` (1.20.1 only)
...
- **Shared game logic** → `common/shared/src/main/java/com/colorchallenge/`
- **Version-specific common code** → `common/<version>/src/main/java/`
- **Fabric-specific code** → `fabric/base/src/main/java/com/colorchallenge/fabric/`
- **NeoForge-specific code** → `neoforge/base/src/main/java/com/colorchallenge/neoforge/`
- **Forge-specific code** → `forge/base/src/main/java/com/colorchallenge/forge/`
...
**1.21.11**: Fabric Loader 0.17.3 / Fabric API 0.141.3+1.21.11, NeoForge 21.11.38-beta
...
Package structure: `com.colorchallenge.{registry,state,entity,block,item,trade,hud,network,event,structure}`
