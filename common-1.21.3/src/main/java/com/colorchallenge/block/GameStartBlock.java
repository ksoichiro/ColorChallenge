...
public class GameStartBlock extends Block {
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos,
        ...
        if (manager.isCountdownActive()) {
            player.displayClientMessage(Component.translatable("message.colorchallenge.countdown_in_progress"), false);
        } else if (manager.getState() == GameState.IN_PROGRESS) {
            player.displayClientMessage(Component.translatable("message.colorchallenge.already_in_progress"), false);
        } else if (manager.getState() == GameState.ENDED) {
            player.displayClientMessage(Component.translatable("message.colorchallenge.game_ended_reset_required"), false);
        }
