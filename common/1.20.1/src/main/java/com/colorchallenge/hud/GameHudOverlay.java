...
public class GameHudOverlay {
    ...
    public static void render(GuiGraphics graphics, float tickDelta) {
        ...
        // Dye delivery count display
        int delivered = ClientGameState.getDeliveredCount();
        String dyeText = String.format("%d / %d", delivered, GameStateManager.TARGET_DYE_COUNT);
        int textWidth = mc.font.width(dyeText);
        int x = screenWidth - textWidth - MARGIN;
        graphics.drawString(mc.font, dyeText, x, y, 0xFFD700, true);
