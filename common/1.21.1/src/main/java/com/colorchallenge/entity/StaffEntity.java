...
public class StaffEntity extends Mob {
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!manager.canDeliver()) {
            player.sendSystemMessage(Component.translatable("message.colorchallenge.game_not_started"));
        ...
        if (heldItem.isEmpty() || !(heldItem.getItem() instanceof DyeItem dyeItem)) {
            player.sendSystemMessage(Component.translatable("message.colorchallenge.hold_dye"));
            setUnhappyCounter(20);
            this.playSound(SoundEvents.VILLAGER_NO, 1.0f, 1.0f);
            return InteractionResult.CONSUME;
        }
        ...
        if (manager.getDeliveredDyes(player.getUUID()).contains(color)) {
            player.sendSystemMessage(Component.translatable("message.colorchallenge.already_delivered", color.getName()));
            setUnhappyCounter(20);
            this.playSound(SoundEvents.VILLAGER_NO, 1.0f, 1.0f);
            return InteractionResult.CONSUME;
        }
