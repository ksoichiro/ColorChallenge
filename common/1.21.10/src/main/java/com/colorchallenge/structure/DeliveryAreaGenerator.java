package com.colorchallenge.structure;
...
public class DeliveryAreaGenerator {
    private static final ResourceLocation TEMPLATE_ID =
            ResourceLocation.fromNamespaceAndPath(ColorChallenge.MOD_ID, "delivery_area");
    ...
    // Y offset from structure origin to player foot level (staffs are at Y=2 in the template)
    private static final int FLOOR_HEIGHT = 2;
    private static final int PLAYER_COUNT = 8;

    /**
     * Place the delivery area NBT structure at the best location near the given X/Z.
     * Searches for a flat, above-sea-level area within a few chunks.
     * Sets the world spawn inside the delivery area.
     *
    ...
    public static boolean generate(ServerLevel level, BlockPos center) {
        ...
        if (template.isEmpty()) {
            ColorChallenge.LOGGER.warn("Delivery area template not found: {}", TEMPLATE_ID);
            return false;
        }
        ...
        // Force-load chunks around the center so heightmap queries return correct values.
        // Without this, chunks may not be generated yet during SERVER_STARTED,
        // causing the structure to be placed underground.
        ensureChunksLoaded(level, center, SEARCH_RADIUS + Math.max(size.getX(), size.getZ()));
        ...
        // Set world spawn inside the delivery area (center, above the floor)
        BlockPos spawnPos = new BlockPos(
                placePos.getX() + size.getX() / 2,
                placePos.getY() + FLOOR_HEIGHT,
                placePos.getZ() + size.getZ() * 3 / 4
        );
        level.setRespawnData(net.minecraft.world.level.storage.LevelData.RespawnData.of(
                net.minecraft.world.level.Level.OVERWORLD, spawnPos, 0, 0));
        ...
        ColorChallenge.LOGGER.info("Delivery area placed at {}, spawn set to {}", placePos, spawnPos);
        return true;
    }
    ...
    private static void fillChestWithEquipment(Container container) {
        ...
        for (int i = 0; i < PLAYER_COUNT; i++) {
            container.setItem(slot++, new ItemStack(ModItems.INSTRUCTIONS.get()));
            container.setItem(slot++, new ItemStack(Items.BREAD, 64));
        }
    }

    private static void ensureChunksLoaded(ServerLevel level, BlockPos center, int radius) {
